...
#define LANE_CLASSES 5
#define PROP_SIZE (LANE_CLASSES + 5 + N_OFFSETS)
...
__device__ inline bool devIoU(scalar_t const * const a, scalar_t const * const b, const float threshold) {
  const int start_a = (int) (a[LANE_CLASSES + 2] * N_STRIPS - DATASET_OFFSET + 0.5); // 0.5 rounding trick
  const int start_b = (int) (b[LANE_CLASSES + 2] * N_STRIPS - DATASET_OFFSET + 0.5);
  const int start = max(start_a, start_b);
  const int end_a = start_a + a[LANE_CLASSES + 4] - 1 + 0.5 - ((a[LANE_CLASSES + 4] - 1) < 0); //  - (x<0) trick to adjust for negative numbers (in case length is 0)
  const int end_b = start_b + b[LANE_CLASSES + 4] - 1 + 0.5 - ((b[LANE_CLASSES + 4] - 1) < 0);
  const int end = min(min(end_a, end_b), N_OFFSETS - 1);
  // if (end < start) return 1e9;
  if (end < start) return false;
  scalar_t dist = 0;
  for(unsigned char i = LANE_CLASSES + 5 + start; i <= LANE_CLASSES + 5 + end; ++i) {
    if (a[i] < b[i]) {
      dist += b[i] - a[i];
    } else {
      dist += a[i] - b[i];
